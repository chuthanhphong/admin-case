<template>
  <div>
    <div class="wrap-content-contract">
      ...
      <v-row>
        <v-col>
          <label class="text--body-1" for="note">{{
            $t("contract.note")
          }}</label>
          <v-textarea
            v-model="note"
...
import DialogConfirmSave from "./DialogConfirmSave.vue";
...
export default {
  ...
  methods: {
    ...
    async saveContract() {
      ...
      if (this.contractDetail) {
        this.parentDocumentId = this.contractDetail.documentId;
      }
      formData.documentFormulaId = this.contractType.id;
      formData.parentDocumentId = this.parentDocumentId;
      ...
      try {
        ...
        setTimeout(() => {
          // this.goBackStep()
          this.$router.replace(`${econtractRoute.LIST_CONTRACT_CREATED_PATH}`);
        }, 1500);
