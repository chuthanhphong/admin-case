<template>
  <v-dialog v-if="signerEdit" v-model="showDialog" persistent max-width="500">
    <v-card class="dialog-edit-signature">
      <v-card-title class="d-flex justify-center flex-column">
        <h5 class="text-center font--extra--bold">
          {{ $t('contract.dialog.configSignature') }}
        </h5>
        <span class="text-primary">{{ signerEdit.name }}</span>

        <v-icon class="close-dialog" @click="closeDialog">mdi-close</v-icon>
      </v-card-title>
      <div class="px-8 py-2 pt-0 text--body-2">
        <v-radio-group v-model="option">
          <v-radio :value="optionsCopy.ALL_PAGE">
            <template v-slot:label>
              <div class="mr-10 font--semi-bold">{{ $t('contract.dialog.copyAllPage') }}</div>
              <v-tooltip right content-class="tooltip-right">
                ...
                <div class="mw-300 pa-0">
                  {{ $t('contract.dialog.noteCopyAllPage') }}
                </div>
              </v-tooltip>
            </template>
          </v-radio>
          <v-radio :value="optionsCopy.SELECT_PAGE">
            <template v-slot:label>
              <div class="mr-10 font--semi-bold">{{ $t('contract.dialog.copyRangePage') }}</div>
              <v-tooltip right content-class="tooltip-right">
                <template v-slot:activator="{ on, attrs }">
                  <v-icon
                    v-bind="attrs"
                ...
                <div class="mw-300 pa-0">
                  {{ $t('contract.dialog.noteCopyRangePage') }}
                </div>
              </v-tooltip>
            </template>
          </v-radio>
        ...
        <v-scroll-y-transition>
          <div
            ...
            class="wrap-select-page-sign text--body-2 col-10 mx-auto pa-0"
          >
            ...
            <div class="wrap-page-sign-footer">
              ...
              <b class="primary--text pl-2 btn" @click="addSelectPageSign">{{
                $t('contract.dialog.addRangePage')
              }}</b>
            </div>
          </div>
        </v-scroll-y-transition>
