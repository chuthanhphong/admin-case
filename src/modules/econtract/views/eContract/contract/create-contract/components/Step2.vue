            ...
            >
              ...
              >mdi-minus-circle-outline</v-icon> -->
                ...
                <div class="col-12 col-sm-4 py-0">
                  ...
                  >
                    <v-text-field
                      ref="myRepresentGroupName"
                      v-model="represent.groupName"
                      outlined
                      dense
                      :placeholder="$t('contract.selectCustomer')"
                      required
                      :error-messages="$t(errorsGroupName(represent.groupName))"
                      @blur="resetFormOnput(index)"
                      @change="resetForm(index);represent.groupName = trimSpace(represent.groupName)"
                      ...
                      <container lock-axis="y" class="bg-light-blue py-1">
                        <validation-observer ref="observerCoordinator">
                          <group-review
                            ...
                            @reset-signatures="setResetSignatures(true)"
                          />
                        </validation-observer>
                      </container>
...
export default {
  ...
  data() {
    return {
      ...
      listSigner: [
        // Dai dien ben tao
        {
          type: constants.GROUPS.INTERNAL,
          data: {
            reviews: [{ ...initSignerData }],
  ...
  watch: {
    ...
    contractDetail() {
      if (!this.contractDetail) {
        ...
        this.listSigner = this.rootListSigner;
  ...
  async mounted() {
    // await this.$refs.observerStep2.validate()
  },
  methods: {
    ...mapActions("contract", [
      "setListSigner",
    ...
    errorsGroupName(val) {
      val = val ? val.trim() : ''
      if (!val) {
        return 'document.customerRepresenError'
      }
      return ''
    },
    trimSpace(val) {
    ...
    isDisplayPartner(value) {
      ...
      if (
        ...
        // console.log('groupPepresents', groupPepresents);
        //   if (groupPepresents) {
    ...
    addSigner(index) {
      if (
        this.listSigner[index].groupName !== null &&
        this.listSigner[index].groupName !== ""
      ) {
