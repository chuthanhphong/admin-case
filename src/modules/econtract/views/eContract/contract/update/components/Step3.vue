<template>
  <div>
    ...
    <!-- Button bottom -->
    <div class="d-flex justify-end align-center bottom-tool pr-3">
      <v-btn
...
import TextPagePdf from "../../create-contract/components/TextPagePdf";
import IconSign from "../../create-contract/components/IconSign";
import { mapState, mapActions } from "vuex";
import TypeSignature from "../../create-contract/components/TypeSignature";
import constants from "@/constants";
import { randomId } from "@/core/utils";
import _ from "lodash";
// import RangPage from './RangPage.vue'
import DialogConfigSignature from "../../create-contract/components/DialogConfigSignature.vue";
import DialogSelectSigner from "../../create-contract/components/DialogSelectSigner.vue";
import TabsSign from "../../create-contract/components/TabsSign.vue";
...
export default {
  ...
  watch: {
    ...
    async fileContract() {
      if (!this.fileContract) return;
      if (!this.fileContract.url) return;

      // Reset lai list chan ky.
      this.clearAllSignature();

      // if (this.contractDetail && this.fileContract) {
      //   if (this.contractDetail.fileMainSign.id === this.fileContract.id) {
      //     this.initListSignature();
      //   }
      // }
    },
  ...
  methods: {
    ...
    async initListSignature() {
      var lstSignatures = [];
      this.contractDetail.representGroups.map((rep) => {
        if (rep.signers && rep.signers.length > 0) {
          rep.signers.map((s) => {
            s.docNoteSigns.map((note) => {
              note.x = note.axisX;
              note.y = note.axisY;
              lstSignatures.push({ ...note, ...s });
            });
          });
        }
      });
      setTimeout(() => {
        lstSignatures.map((signer) => {
          try {
            this.addItemSign2(
              "img-sign",
              signer,
