<template>
  <div>
    <div
      class="d-flex detail-task-chirldren align-center justify-start"
    >
      <div class="d-flex align-center justify-center pr-0 cursor-pointer pa-0 clear-display">
        <v-tooltip
          content-class="tooltip-top"
          top
        >
          <template v-slot:activator="{ on }">
            <div class="d-flex flex-column hover--pointer" @click="getTaskDetail">
              <img
                ...
                @click="getTaskDetail"
                v-on="formData.name ? on : null"
              >
            </div>
          </template>
          <div>
            {{ $t('task-manager.tool-tip.view-task') }}
          </div>
        </v-tooltip>
      </div>
      <div class="mt-2 ml-2">
        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-text-field
              ...
              :readonly="!isDisable"
              class="text--body-5 text-capitalize"
              dense
              height="24px"
              maxlength="200"
              name="name"
              v-bind="formData.name ? attrs : ''"
              @change="formData.name = trimSpace(formData.name)"
              v-on="formData.name ? on :''"
            />
          </template>
          <span>{{ formData.name }}</span>
        </v-tooltip>
      </div>
      <div class="d-flex align-center justify-center select-box-status mt-3">
        <v-select
          ...
          :menu-props="{ maxHeight: '400', maxWidth:'235' }"
          :readonly="!isDisable"
          append-icon=""
          class="label-chip-status-task"
          dense
        >
          ...
          <template v-slot:item="data">
            <template>
              <div class="select--status d-flex flex-row align-center">
                ...
                <v-list-item-title class="ml-2" style="width: 180px!important;" v-html="$t(data.item.label)" />
              </div>
            </template>
          </template>
        </v-select>
      ...
      <div class="d-flex align-center justify-center pr-4">
        <v-tooltip content-class="tooltip-top" top>
          <template v-slot:activator="{ on }">
            <div class="d-flex flex-column">
              <div class="justify-center" v-on="on">
                <v-menu
          ...
          <div>
            {{ listDatePicker.length !== 0 ? getToolTipDate() : $t('task-manager.tool-tip.deadline') }}
          </div>
        </v-tooltip>
        <v-menu
        ...
        >
          <template v-slot:activator="{ on: menu, attrs }">
            <v-tooltip content-class="tooltip-top" top>
              <template v-slot:activator="{ on: tooltip }">
                <div class="d-flex flex-column ml-3">
                  ...
                  <div v-else class="justify-center">
                    <v-img
                      :src="require('@/assets/icons/task/detailTask/Group.svg')"
                      alt=""
                      class="hover--pointer"
                      v-bind="attrs"
                      v-on="{ ...tooltip, ...menu }"
              ...
              <div>
                {{ formData.performer ? formData.performer.userFullName : $t('task-manager.tool-tip.coordinator') }}
              </div>
            </v-tooltip>
          </template>
          <v-autocomplete
        ...
        <span v-if="isDisable && checkCurrentUserId(formData.creatorId)" class="cursor-pointer ml-1" @click="onRemove">
...
export default {
  ...
  computed: {
    ...
    errorTaskName() {
      let str = '';
      (this.listDatePicker.length !== 0 || this.formData.performer) && (!this.formData.name)
  ...
  methods: {
    ...
    renderStatus(value) {
      for (let i = 0; i < this.listStatusTask.length; i++) {
        if (this.listStatusTask[i].value === value) {
          return this.listStatusTask[i].label
        }
      }
    },
    accept() {
    ...
    getToolTipDate() {
      if (this.listDatePicker.length === 0) {
        return ''
      } else {
        if (this.listDatePicker[0]) {
          if (this.listDatePicker[1]) {
            const startDate = this.formatDate(this.listDatePicker[0])
            const endDate = this.formatDate(this.listDatePicker[1])
            return startDate + ' - ' + endDate
          } else {
            return this.formatDate(this.listDatePicker[0])
          }
        }
      }
    }
