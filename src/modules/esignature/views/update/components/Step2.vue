<template>
  <div>
    ...
    <validation-observer ref="observerStep2" v-slot="{ invalid }">
      <div class="esignature-wrap-content-step-2">
        <div class="wrap-types">
          <container lock-axis="y" :drag-handle-selector="!isDisbale ? '' : 'column-drag-handle'" @drop="onDropListSigner">
            <draggable v-for="(signer, index) in listSigner" :key="index">
              ...
              <v-card class="mb-10 pb-5 group-sign-esignature rounded-xl group-blue">
                <div class="header-panel d-flex align-center pr-6">
                  <v-tooltip
                    v-if="listSigner.length > 1 && !isDisbale"
                    top
                    ...
                    <v-expansion-panel class="elevation-0 rounded-xl">
                      <v-expansion-panel-content class="mx-8 pb-3 rounded-lg">
                        <container
                          ...
                          :drag-handle-selector="!isDisbale ? '' : 'column-drag-handle'"
...
export default {
  ...
  computed: {
    ...mapState('esignature', ['documentDetail', 'parentId']),
  ...
  watch: {
    documentDetail() {
      ...
      const representGroups = this.documentDetail.signatureSigners
      this.listSigner = []
      const groupLevel = _.groupBy(representGroups, 'groupLevel')
      const signatureSigners = Object.values(groupLevel)
      signatureSigners.map((signers) => {
        if (signers.length > 0) {
          signers.map((s) => {
            s.signUserId = s.signUserId || null
            if (s.signMethod === constants.SIGN_METHODS.ALL) {
              s.signMethod = [
                constants.SIGN_METHODS.IMAGE_SIGN,
                constants.SIGN_METHODS.DIGITAL_SIGN
              ]
            } else if (s.signMethod === constants.SIGN_METHODS.NORMAL_SIGN) {
              s.signMethod = []
            } else {
              s.signMethod = [s.signMethod]
            }
          })
        }
        const item = {
