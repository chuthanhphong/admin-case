<template>
  <v-dialog v-if="signerEdit" v-model="showDialog" persistent max-width="680">
    <v-card class="dialog-edit-signature">
      <v-card-title class="d-flex justify-center flex-column">
        <h5 class="text-center">
          <div>{{ $t('contract.dialog.configSignature') }}</div>
        </h5>
      ...
      <div class="px-8 py-2 pt-0 text--body-2">
        <v-radio-group v-model="option">
          <v-radio :value="optionsCopy.ALL_PAGE">
            <template v-slot:label>
              <div class="mr-10">{{ $t('contract.dialog.copyAllPage') }}</div>
          ...
          <v-radio :value="optionsCopy.SELECT_PAGE">
            <template v-slot:label>
              <div class="mr-10">{{ $t('contract.dialog.copyRangePage') }}</div>
              <v-tooltip right content-class="tooltip-right">
                <template v-slot:activator="{ on, attrs }">
                  <v-icon
                    v-bind="attrs"
        ...
        <v-scroll-y-transition>
          <div
            ...
            class="wrap-select-page-sign text--body-2 col-10 mx-auto"
          >
