<template>
  <div class="wrap--list-common edocument-font wrap-create-document">
    <v-container fluid class="py-0">
      ...
      />
      <dialog-confirm-save
        title-confirm="managementDoc.titleConfirmSave.saveDocument"
...
export default {
  ...
  computed: {
    breadcrumbs() {
      ...
      const title = 'document.bookTitle'
  ...
  async created() {
    this.initDate()
    const document = this.$route.query

    if (!isNull(document)) {
      if (document.document) {
        this.typeDocument = document.document
      }
    }
  ...
  methods: {
    ...
    async getSuggestNumber() {
      try {
        ...
        this.documentNumber = response.data
    ...
    async getBookGroup() {
      try {
        ...
        if (this.lstGroupBook) {
          this.bookGroupId = this.lstGroupBook[0].groupId
          this.bookGroupCode = this.lstGroupBook[0].groupCode
          this.promulgateGroupName = this.lstGroupBook[0].groupName
          this.promulgateGroupId = this.lstGroupBook[0].groupId
    ...
    async saveSignatureDocument() {
      try {
        ...
        var message = this.$t('managementDoc.messageSuccess.saveDocumentGo')
        let curren = 0
        if (this.checkTypeDocument) {
          message = this.$t('managementDoc.messageSuccess.saveDocumentTo')
          curren = 1
        }
        this.setNotify({
          show: true,
        ...
        this.$router.push(`${edocumentRoute.DOCUMENT_BOOK_PATH}?currentTab=${curren}`)
      } catch (error) {
        this.setNotify({
          show: true,
          type: 'error',
...
<style lang="scss" src="./CreateBook.scss"></style>
