...
export default {
  ...
  data() {
    return {
      ...
      addInitSigner: false,
      listSignDefault: [], // list chan ky khong duoc xoa.
    };
  },
  computed: {
    ...mapState("contract", [
    ...
    clearAllSignature() {
      if (this.itemsSign.length > 0) {
        ...
        this.itemsSign.map((item) => {
          if (!item.signer.userId && item.$el) {
            item.$el.remove();
          }
        });
        // Xoa item
        this.itemsSign = this.itemsSign.filter((item) => item.signer.userId);
      }
    },
      ...
      setTimeout(() => {
        lstSignatures.map((signer) => {
          try {
            ...
            this.listSignDefault.push(signer.id);
          } catch (error) {
            console.log(error);
          }
        });
