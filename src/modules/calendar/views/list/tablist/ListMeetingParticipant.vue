  ...
  <div class="calendar-font wrap-content-calendar-search">
    ...
    <div v-if="showListData" class="calendar-content-list mt-4">
      ...
      <div :class="lstMeeting.length > 0 ? '' : 'd-none'">
        <v-data-table
          id="virtual-scroll-table-participant"
          ref="tableParticipant"
          v-scroll:#virtual-scroll-table-participant="onScroll"
          hide-default-footer
          fixed-header
          group-by="dateValue"
...
import DialogConfirmSave from "@/modules/calendar/views/components/DialogConfirmSave.vue";
...
const initData = {
  ...
  status: ["APPROVED"], // trang thai: DELETED, DRAFT, PENDING ,APPROVED, REJECTED, CANCELED, EXPIRED
  meetingTitle: null,
...
export default {
  ...
  computed: {
    computedHeightTable() {
      const box = document.getElementById("virtual-scroll-table-participant");
  ...
  methods: {
    ...
    async search() {
      try {
        ...
        this.$emit("count-participant", this.totalCount);
    ...
    onScroll(e) {
      ...
      //     "#virtual-scroll-table-participant > .v-data-table__wrapper tbody"
      //   ).offsetHeight;
    ...
    onDetailMeeting(item) {
      if (
        ...
        localStorage.setItem(
          ...
          constants.CALENDAR_TAB_LIST.ZERO
        );
        this.$router.push(
