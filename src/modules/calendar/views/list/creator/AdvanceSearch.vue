<template>
  <v-layout column class="calendar-font">
    ...
    <v-layout column class="scroll-advance-search-options overflow-y-auto px-6">
      ...
      <v-layout column>
        <label class="calendar-text--body-4 text-uppercase mb-3">
          {{ $t('calendar.advanceSearch.timeCreate') }}
          ...
          <div>
            <v-layout column>
              ...
              >
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field
                    v-model="createStartTimeValue"
                    outlined
                    placeholder="DD/MM/YYYY"
                    dense
                    v-bind="attrs"
                    class="calendar-text--body-4 mw-175"
                    :error-messages="$t(errorCreateStartTime)"
                    v-on="on"
                    @click:append="menuCreateStartTime = !menuCreateStartTime"
                  >
                    <template v-slot:append>
                      <i
                        class="app-icon icon-calendar darken"
                        @click="menuCreateStartTime = !menuCreateStartTime"
                      ></i>
                    </template>
                  </v-text-field>
                </template>
                <v-date-picker
                  v-model="createStartTimePicker"
                  :locale="localDate"
                  no-title
                  show-adjacent-months
                  @input="menuCreateStartTime = false"
                />
              </v-menu>
            </v-layout>
          </div>
          <div>
            <v-layout column>
              ...
              >
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field
                    v-model="createEndTimeValue"
                    outlined
                    placeholder="DD/MM/YYYY"
                    dense
                    v-bind="attrs"
                    class="calendar-text--body-4 mw-175"
                    :error-messages="$t(errorCreateEndTime)"
                    v-on="on"
                    @click:append="menuCreateEndTime = !menuCreateEndTime"
                  >
                    <template v-slot:append>
                      <i
                        class="app-icon icon-calendar darken"
                        @click="menuCreateEndTime = !menuCreateEndTime"
                      ></i>
                    </template>
                  </v-text-field>
                </template>
                <v-date-picker
                  v-model="createEndTimePicker"
                  :locale="localDate"
                  no-title
                  show-adjacent-months
                  @input="menuCreateEndTime = false"
                />
              </v-menu>
            </v-layout>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      // thoi gian tao from.
      menuCreateStartTime: null,
      errorCreateStartTime: null,
      createStartTimePicker: null,
      createStartTimeValue: null,

      // thoi gian tao to.
      menuCreateEndTime: null,
      errorCreateEndTime: null,
      createEndTimePicker: null,
      createEndTimeValue: null,
  ...
  watch: {
    ...
    // thoi gian tao
    createStartTimePicker(date) {
      if (date) this.createStartTimeValue = this.formatDate(date)
    },

    createEndTimePicker(date) {
      if (date) this.createEndTimeValue = this.formatDate(date)
    },

    createStartTimeValue() {
      if (this.createStartTimeValue) {
        this.validateCreateStartTime()
        this.validateCreateEndTime()
      } else {
        this.meetingEndTimePicker = null
        this.errorCreateStartTime = 'messError.dateRequired'
      }
      this.checkDisbaleButton()
    },

    createEndTimeValue() {
      if (this.createEndTimeValue) {
        this.validateCreateStartTime()
        this.validateCreateEndTime()
      } else {
        this.createEndTimePicker = null
        this.errorCreateEndTime = 'messError.dateRequired'
      }
      this.checkDisbaleButton()
    },
    roomName() {
  ...
  methods: {
    ...
    checkDisbaleButton() {
      this.isComplete =
        ...
        isNull(this.errorCreateStartTime) &&
        isNull(this.errorCreateEndTime)
    },
    ...
    validateMeetingStartTime() {
      ...
      if (!isValid) {
        this.errorMeetingStartTime = 'calendar.messError.dateCreatedInvalid'
        this.meetingStartTimePicker = null
        return
      }
      ...
      if (isValid) {
        ...
        if (
          ...
          this.errorMeetingEndTime = 'calendar.messError.dateCreatedInvalid'
          return
        }
    ...
    // validate thoi gian tao
    validateCreateStartTime() {
      var isValid = moment(
        this.createStartTimeValue,
        'DD/MM/YYYY',
        true
      ).isValid()

      if (!isValid) {
        this.errorCreateStartTime = 'calendar.messError.dateCreatedInvalid'
        this.createStartTimePicker = null
        return
      }

      if (isValid) {
        this.createStartTimePicker = moment(
          this.createStartTimeValue,
          'DD/MM/YYYY'
        ).format('YYYY-MM-DD')

        const dateTo = moment(this.createEndTimeValue, 'DD/MM/YYYY')
        const dateFrom = moment(this.createStartTimeValue, 'DD/MM/YYYY')
        const diff = dateTo.diff(dateFrom, 'days')
        this.errorCreateStartTime = null

        if (!moment(this.createEndTimeValue, 'DD/MM/YYYY', true).isValid()) {
          this.errorCreateEndTime = 'calendar.messError.dateCreatedInvalid'
        ...
        if (diff < 0) {
          this.errorCreateEndTime = 'messError.compareDate'
          this.createStartTimePicker = ''
          return
        }

        if (diff > 365) {
          this.errorCreateEndTime = 'messError.dateExpire'
          this.createStartTimePicker = ''
          return
        }

        this.errorCreateStartTime = null
        this.errorCreateEndTime = null

        if (this.createEndTimePicker) this.errorCreateEndTime = null
      }
    },
    validateCreateEndTime() {
      var isValid = moment(
        this.createEndTimeValue,
        'DD/MM/YYYY',
        true
      ).isValid()
      ...
      if (isValid) {
        this.createEndTimePicker = moment(
          this.createEndTimeValue,
          'DD/MM/YYYY'
        ).format('YYYY-MM-DD')
        ...
        if (!moment(this.createStartTimeValue, 'DD/MM/YYYY', true).isValid()) {
          this.errorCreateStartTime = 'calendar.messError.dateCreatedInvalid'
        ...
        if (this.createStartTimePicker) this.errorCreateStartTime = null
      }
    },
    ...
    initDate() {
      this.createStartTimeValue = moment()
        .subtract(365, 'days')
        .format('DD/MM/YYYY')
    ...
    clearFilter() {
      ...
      this.roomId = null
      this.initDate()
      this.meetingStartTimeValue = null
      this.meetingEndTimeValue = null
      this.listIndexState = [
        'DRAFT',
        'PENDING',
        'APPROVED',
    ...
    onSubmit() {
      const params = {
        keyword: '',
        meetingStartTime: this.meetingStartTimeValue
          ? this.meetingStartTimeValue.replaceAll('/', '')
          : null, // thoi gian bat dau hop
        meetingEndTime: this.meetingEndTimeValue
          ? this.meetingEndTimeValue.replaceAll('/', '')
          : null, // Thoi gian ket thuc hop
        createStartTime: this.createStartTimeValue.replaceAll('/', ''), // thoi gian tao tu
        createEndTime: this.createEndTimeValue.replaceAll('/', ''), // Thoi gian tao den
        status:
          this.listIndexState.length > 0
            ? this.listIndexState
            : [
              'DRAFT',
              'PENDING',
              ...
              'EXPIRED',
              'CANCELED'
            ], // trang thai: DELETED, DRAFT, PENDING ,APPROVED, REJECTED, CANCELED, EXPIRED
        meetingTitle: this.formdata.meetingTitle,
        meetingTypes: this.genTypeCalenar(), // loai lich: MEETING_SCHEDULE, ON_CALL_SCHEDULE
