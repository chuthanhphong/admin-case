  ...
  <div class="wrap-home ml-2 mt-2 contracts-page">
    <v-container fluid class="py-5 pt-2 rounded-xl bg-primary pb-5">
      <h4 class="white--text ml-2">
        {{ $t('contract.label.pendingContract') }}
      </h4>
      <v-card
        color="white"
                  ...
                  <template v-slot:default>
                    ...
                    <tbody class="text--body-5">
                      ...
                      >
                        ...
                        <td class="text-left">{{ item.sendDate }}</td>
...
import SearchContractProcess from '../components/process/SearchContractProcess.vue'
export default {
  components: {
    DialogContractDetail,
    DialogConfirm,
    EmptyBox,
    SearchContractProcess
  },
  ...
  data() {
    return {
      ...
      formdata: {
        ...
        keyword: '',
        searchType: constants.SEARCH_TYPE_CONTRACT.PROCESS_CONTRACT,
        fromDate: null,
        toDate: null,
        effectDateFrom: null,
        effectDateTo: null,
  ...
  methods: {
    ...
    renderDocState(docState) {
      if (docState) {
        return this.$t(`contract.label.docStates.${docState.toLowerCase()}`)
      }
    },
    isValidDate(strDate) {
    ...
    renderStatus(status) {
      switch (status.signUserType) {
        case constants.SIGN_USER_TYPE.MAIN: // ky chinh
          switch (status.processStatus) {
            case constants.SIGN_PROCCESS_STATUS.UNPROCESSED: // Cho ky
              return 'contract.statusMain.unprocessed'
            ...
            case constants.SIGN_PROCCESS_STATUS.REJECTED: // Tu choi
              return 'contract.statusMain.reject'
            default:
              return ''
          }
        ...
        case constants.SIGN_USER_TYPE.REVIEWER: // Xem xet
          switch (status.processStatus) {
    ...
    async search() {
      try {
        if (!this.showAdvance) {
          ...
          this.formdata.typeContract = null
