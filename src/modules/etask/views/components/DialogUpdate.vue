            ...
            <div :class="formData.name ?'' : 'opacity-3'" class="dialog-body">
              ...
              <div v-if=" isShowSourceTask" id="sourceTask" class="d-flex flex-column">
                ...
                <dialog-select-source-task
                  :excludes="[{ id:formData.id, associationType: 'TASK' }]"
              ...
              <div class="mt-1 d-flex flex-column py-2">
                ...
                <div class="d-flex align-center ml-7 cursor-pointer">
                  <div class="d-flex align-center" @click="formData.name ? addChildWork() : ''">
                    <v-btn
                      elevation="0"
                      fab
                      x-small
                ...
                <div class="d-flex justify-space-between align-start pl-8">
                  ...
                  >
                    ...
                    <ul class="list-files text--caption">
                      <li v-for="(file, index) in formData.fileAttachments" :key="index">
                        <img
            ...
            <div :class="formData.name ?'' : 'opacity-3 mt-2'" class="mt-2">
              ...
              <v-card-actions class="justify-end mt-2">
                ...
                <v-btn
                  ...
                  color="primary"
                  rounded
                  @click="showDialogConfirmUpdateTask"
                >
...
import Constants from '@/modules/etask/constants';
import ChildrenWorkTask from "@/modules/etask/views/components/ChildrenWorkTask";
...
const initChildWorkData = {
  ...
  performer: null,
  creatorId: '',
  status: constants.STATUS_TASK.NEW
}
export default {
  ...
  computed: {
    ...
    validateAssigner() {
      let str = ''
      this.formData.status !== constants.STATUS_TASK.NEW && !this.formData.assigner ? str = 'task-manager.error.assigner' : str = ''
      return str
    },
    validatePerformer() {
      let str = ''
      this.formData.status !== constants.STATUS_TASK.NEW && !this.formData.performer ? str = 'task-manager.error.perfomer' : str = ''
      return str
    },
    validateDate() {
      let str = ''
  ...
  watch: {
    ...
    showDialog() {
      if (this.showDialog) {
        ...
        this.getWorkType()
        this.formData.status === constants.STATUS_TASK.NEW ? this.statusBengin = this.formData.status : this.statusBengin = ''
        this.formDataAfter = this.formData
        if (this.formData.fileAttachments === null) {
          this.formData.fileAttachments = []
        }
        if (this.formData.childrenTask === null) {
    ...
    "formData.id"() {
      ...
      if (this.formData.endDate) {
        if (this.formData.startDate) {
          ...
          this.deadlineFromPicker[0] = moment(this.formatDateStart, 'DD/MM/YYYY').format('YYYY-MM-DD')
          this.deadlineFromPicker[1] = moment(this.formatDateEnd, 'DD/MM/YYYY').format('YYYY-MM-DD')
          ...
          const endDate = this.formData.endDate ? this.formData.endDate : ''
          this.formatDateEnd = endDate.split(' ')[0]
          this.deadlineFromPicker[0] = moment(this.formatDateEnd, 'DD/MM/YYYY').format('YYYY-MM-DD')
          this.endTime = endDate.split(' ')[1]
          this.formatDateStart = ''
          this.startTime = ''
          const vi = 'task-manager.label.endDateStr'
          this.strDeadline = I18n.t(vi, { end: this.formData.endDate })
        }
      } else {
        this.formatDateStart = ''
        this.formatDateEnd = ''
    ...
    formData: {
      handler() {
        ...
        if (this.isInit) {
          this.formData.combinators ? this.isShowPerfomer = true : this.isShowPerfomer = false
          this.formData.monitors ? this.isShowFollower = true : this.isShowFollower = false
          this.formData.startDate || this.formData.endDate ? this.isShowDeadline = true : this.isShowDeadline = false
          this.formData.taskCategoryId ? this.isShowTag = true : this.isShowTag = false
          this.formData.priority ? this.isShowflash = true : this.isShowflash = false
          this.formData.fileAttachments !== [] ? this.isShowFileInput = true : this.isShowFileInput = false
          this.formData.sourceTask ? this.isShowSourceTask = true : this.isShowSourceTask = false
          this.formData.taskAssociations ? this.isShowlink = true : this.isShowlink = false
          if (this.formData.status === constants.STATUS_TASK.FINISH) {
            this.isShowDeadline = true
          }
        }
  ...
  methods: {
    ...
    formatDateFromArray() {
      const startDate = this.deadlineFromPicker[0]
      const endDate = this.deadlineFromPicker[1]
      this.formatDateStart = this.deadlineFromPicker[0] ? this.formatDate(startDate) : ''
      this.formatDateEnd = this.deadlineFromPicker[1] ? this.formatDate(endDate) : ''
      const vi = 'task-manager.label.deadlinestr'
      const viEnd = 'task-manager.label.endDateStr'
      this.deadlineFromPicker[1] ? this.strDeadline = I18n.t(vi, {
        start: this.formatDateStart,
        end: this.formatDateEnd
      }) : this.strDeadline = I18n.t(viEnd, { end: this.formatDateEnd })
    },
    checkAvatar(item, type) {
      return item.associationType === type;
    },
    checkEmptyTaskName() {
    ...
    async updateTask() {
      if (this.deadlineFromPicker[0]) {
        const startDate = this.formatDate(this.deadlineFromPicker[0])
        if (this.deadlineFromPicker[1]) {
          this.startTime ? this.formData.startDate = startDate + ' ' + this.startTime : this.formData.startDate = startDate + ' 00:00'
          const endDate = this.formatDate(this.deadlineFromPicker[1])
          this.endTime ? this.formData.endDate = endDate + ' ' + this.endTime : this.formData.endDate = endDate + ' 23:59'
        } else {
          const endDate = this.formatDate(this.deadlineFromPicker[0])
          this.endTime ? this.formData.endDate = endDate + ' ' + this.endTime : this.formData.endDate = endDate + ' 23:59'
          this.formData.startDate = null
        }
      } else {
      ...
      try {
        this.loading = true
        await TaskService.updateTaskDraff(this.formData)
        this.closeDialog()
    ...
    async selectAttachs() {
      try {
        ...
        this.formData.fileAttachments.map((file, index) => {
          let count = 0;
    ...
    async uploadFileAttachs() {
      ...
      try {
        ...
        if (response.success) {
          const { data } = response
          const lstFileName = this.formData.fileAttachments.map(
            file => file.fileName || file.name
          )
          const lstFileNameWithId = this.formData.fileAttachments.map(file => {
          ...
          data.forEach(file => {
            if (this.isUpdate) {
              ...
              if (!isExited) {
                this.formData.fileAttachments.push(file)
              } else {
                const index = this.formData.fileAttachments.findIndex(
                  x => x.fileName === file.fileName && !x.id
                )
                this.formData.fileAttachments[index] = file
              }
            } else {
    ...
    removeFileAttach(index) {
      ...
      this.formData.fileAttachments.splice(index, 1)
    },
    uploadAttachs() {
    ...
    async downloadFileAttach(index) {
      const attach = this.formData.fileAttachments[index];
