        ...
        <div v-for="(represent, index) in listSigner" :key="index">
          ...
          >
            <div class="header-panel d-flex align-center pr-6">
              ...
              <!-- <span class="title-panel">{{ titlePanel(represent) }}</span> -->
              <v-spacer />
              <div
              ...
              <div class="col-12 col-sm-4 py-0">
                ...
                <div v-if="isDisplayPartner(represent)" class="wrap-users-list">
                  <validation-provider
                    v-slot="{ errors }"
                    name="myRepresent"
                    rules="required"
            ...
            <div class="content-panel">
              ...
              <v-expansion-panels
                ...
                v-model="represent.data.openReview"
                multiple
                class="elevation-0 panel-reviewer"
              >
                <v-expansion-panel class="elevation-0 rounded-xl">
                    ...
                    >
                      ...
                      >
                        <validation-observer ref="observerReviews">
                          <group-review
                            :id="idx"
                            :disabled="!represent.hasPermission"
                            :group="groups"
                            :is-review="true"
                            :group-id="represent.groupId"
                            :show-remove="
                              represent.data.reviews.length > 1 &&
                                !isDisplayGroup(represent.type)
                            "
                            :signer="signer"
                            :list-users="listUsers"
                      ...
                      >
                        <validation-observer ref="observerSigner">
                          <group-signer
                            ...
                            :is-review="false"
                            :show-remove="
                              represent.data.signers.length > 1 &&
                                !isDisplayGroup(represent.type)
                            "
                            :signer="signer"
                            :type="checkType(represent.type)"
...
import BtnAdd from "../../create-contract/components/BtnAdd";
...
export default {
  ...
  computed: {
    ...
    ...mapState("contract", ["contractDetail"]),
  },
  watch: {
    contractDetail() {
      var maxStep = 0;
      ...
      representGroups.map((rep) => {
        ...
        const item = {
          ...
          data: {
            ...
            openReview: reviewers.length > 0 ? [0] : [],
            signers: signers.length > 0 ? signers : [{ ...initSignerData }],
            group: rep.groupId
              ? constants.GROUPS.GROUP
              : constants.GROUPS.PERSONAL,
            isCoordinator:
              !rep.hasPermissionToCoordinate &&
              coordinator &&
              signers.length === 0,
          ...
          openPanel: [0],
          hasPermission: rep.hasPermissionToCoordinate,
          showFilterUser:
  ...
  methods: {
    ...
    validateEmail() {
      ...
      this.listSigner.map((element) => {
        ...
        if (!element.hasPermission && !element.data.signers[0].email) {
