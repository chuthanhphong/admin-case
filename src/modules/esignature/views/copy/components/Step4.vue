<template>
  <div class="esignature-font">
    <div class="esignature-wrap-content">
      <div class="dialog-esignature-detail wrap-types">
        ...
        <div class="col-12 row justify-space-around">
          <div class="form-group col-10">
            <div class="col-12">
              <label>{{ $t('electronicSign.create.nameDocument') }}</label>
        ...
        <div class="col-12 row justify-space-around">
          <div class="form-group col-12 col-sm-6 col-md-4">
            <div class="col-12">
              <label>{{ $t('electronicSign.create.documentForm') }}</label>
              <div class="detail-content">
          ...
          <div class="form-group col-12 col-sm-6 col-md-4">
            <div class="col-12">
              <label>{{ $t('electronicSign.create.urgency') }}</label>
              <div class="detail-content">
        ...
        <div v-if="promulgateGroupId" class="col-12 row justify-space-around">
          <div class="form-group col-12 col-sm-6 col-md-4">
            <div class="col-12">
              <label>{{ $t('electronicSign.create.issueGroup') }}</label>
              <div class="detail-content">
        ...
        <div class="col-12 row justify-space-around">
          <div class="form-group col-12 col-sm-6 col-md-4">
            <div class="col-12">
              <label>{{ $t('document.label.fileSign') }}</label>
              <div
          ...
          <div class="form-group col-12 col-sm-6 col-md-4">
            <div v-if="showFileAttach()" class="col-12">
              <label>{{ $t('document.label.fileAttachs') }}</label>
              <div
                class="d-flex justify-space-between align-center detail-content"
              >
                <ul class="list-files">
      ...
      <v-row>
        <v-col>
          <label class="text--body-1-normal" for="note">{{ $t('document.note') }}</label>
          <v-textarea
            v-model="note"
            rows="3"
            outlined
      ...
      <v-row justify="space-around">
        <v-col cols="12" sm="4" md="3">
          <v-row justify="space-between">
            <v-col>
              <v-row justify="space-between">
                <label class="text--body-1-normal" for="note">{{ $t('document.timeSign') }}</label>
                <v-tooltip top content-class="tooltip-top">
                  <template v-slot:activator="{ on, attrs }">
                    <v-icon
                      v-bind="attrs"
                  ...
                  <div class="mw-300 pa-0">
                    {{ $t('document.tooltip.timeExpired') }}
                  </div>
                </v-tooltip>
              </v-row>
            </v-col>
          </v-row>
          <v-row class="pt-2 date-picker">
            <v-menu
              ref="menu"
            ...
            >
              <template v-slot:activator="{ on, attrs }">
                <v-text-field
                  ...
                  v-on="on"
                  @click:append="menuTimeExpired = !menuTimeExpired"
                />
              </template>
              <v-date-picker
                v-model="timeExpired"
        ...
        <v-col class="d-flex" md="6">
          <v-row align="center">
            ...
            <label class="text--body-1-normal theme--light">{{ $t('document.day') }}</label>
          </v-row>
        </v-col>
      </v-row>
    </div>
...
import DialogConfirmSave from '@/modules/esignature/views/create/components/DialogConfirmSave.vue'
    ...
    confirmSaveContract(docState) {
      ...
      this.titleConfirmSave =
        docState === this.docStates.DRAFT
          ? 'document.dialog.titleConfirmSaveDraft'
          : 'document.dialog.titleConfirmSaveContract'
    },
    // Luu hop dong
    async saveContract() {
      const formData = {}
      formData.processType = constants.PROCESS_TYPE.VB_KY
      ...
      try {
        this.loading = true
        await DocumentService.saveDocument(formData)
        this.loading = false
        var message =
          this.docState === constants.DOC_STATES.DRAFT
            ? 'document.saveDraftSuccess'
            : 'document.saveContractSuccess'
