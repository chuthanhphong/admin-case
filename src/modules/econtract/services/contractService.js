...
export class ContractService extends BaseService {
  ...
  static async getList() {
    try {
      ...
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.MANAGER_CONTRACT_TYPE}?${formatQuery(params)}`
      )
  ...
  static async getRelateDocument(groupData) {
    try {
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.RELASTE_DOCUMENT}?${formatQuery(
          groupData
        )}&deviceType=WEB`
      )
      return new ResponseWrapper(response, response.data.data)
  ...
  static async getFileSign(contractId) {
    try {
      ...
      throw new ErrorWrapper(error, responseJson)
    }
  }
  ...
  // Lay danh sach don vi -> quan ly hop dong
  static async getListGroupContractManager() {
    try {
      const params = {
        pageSize: 100000
      }
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.MANAGER_GROUPS}?${formatQuery(params)}`
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
      const message = error.response.data.data.message
  ...
  // Luu hop dong -> quan ly hop dong
  static async saveContractManager(formData) {
    try {
      const response = await this.requestContract({ auth: true }).post(
        Apis.CONTRACT.MANAGER_ADD_CONTRACT,
        formData
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
  ...
  // Danh sach loai hop dong -> quan ly hop dong
  static async getListFormulaType() {
    try {
      const params = {
        pageSize: 100000
      }
  ...
  // Tim kiem cong ty -> quan ly hop dong
  static async searchCompanyContractManager(params) {
    try {
      const defaultParams = {
      ...
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.MANAGER_SEARCH_COMPANY}?${formatQuery(params)}`
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
      const message = error.response.data
  ...
  // Tim kiem hop dong -> quan ly hop dong
  static async searchContractManager(params) {
    try {
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.MANAGER_SEARCH_CONTRACT}?${formatQuery(params)}`
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
      const message = error.response.data
  ...
  // Tim kiem khach hang -> quan ly hop dong
  static async searchCustomerContractManager(params) {
    try {
      const defaultParams = {
        restrictHierarchy: 'false',
        groupType: 'EXTERNAL',
      ...
      params = { ...params, ...defaultParams }
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.MANAGER_SEARCH_CUSTOMER}?${formatQuery(params)}`
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
      const message = error.response.data
  ...
  // Lay thong tin chi tiet hop dong -> quan ly hop dong
  static async getDetailExternalContract(contractId) {
    try {
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.MANAGER_CONTRACT_DETAIL}/${contractId}`
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
      const message = error.response.data
  ...
  // Lay thong tin chi tiet hop dong update -> quan ly hop dong
  static async getExternalContractInfo(contractId) {
    try {
      const response = await this.requestContract({ auth: true }).get(
        `${Apis.CONTRACT.MANAGER_CONTRACT_INFO_DETAIL}/${contractId}`
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
      const message = error.response.data
  ...
  // Xoa hop dong -> quan ly hop dong
  static async deleteExternalContract(contractId) {
    try {
      const response = await this.requestContract({ auth: true }).put(
        `${Apis.CONTRACT.MANAGER_DELETE_CONTRACT}/${contractId}`
      )
      return new ResponseWrapper(response, response.data.data)
    } catch (error) {
      const message = error.response.data
