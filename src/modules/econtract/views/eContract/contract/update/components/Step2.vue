                ...
                >
                  <template v-slot:activator="{ on, attrs }">
                    <v-icon
                      ...
                      v-bind="attrs"
                      :disabled="checkDisabledCoordinator(represent)"
                ...
                >
                  <v-expansion-panel class="elevation-0 rounded-xl">
                    <v-expansion-panel-header class="mx-2">
                      <span
                        class="pl-5 text--body-1"
                      >{{ $t("contract.coordinator") }}
                        <span class="color-require">*</span></span>
                    ...
                    <v-expansion-panel-content class="mx-3 pb-3 rounded-lg">
                      <container
                        ...
                        :drag-handle-selector="
                          checkDisabledCoordinator(represent)
                            ? '.from-group-sign-block'
                            : '.from-group-sign'
                        "
                        ...
                        >
                          <validation-observer ref="observerReviews">
                            <group-review
                              ...
                              :is-review="true"
                              :group-id="represent.groupId"
                              :show-remove="showRemoveReviewer(represent)"
                              :signer="signer"
                              :list-users="listUsers"
                      ...
                      <container
                        ...
                        :drag-handle-selector="
                          checkDisabledCoordinator(represent)
                            ...
                            : '.from-group-sign'
                        "
                        @drop="onDropSigners(index, $event, represent)"
                      >
                        <draggable
                        ...
                        >
                          <validation-observer ref="observerSigner">
                            <group-signer
                              ...
                              :is-review="false"
                              :show-remove="showRemoveSigner(represent)"
                              :signer="signer"
                              :type="checkType(represent.type)"
                              :total-signers="represent.data.signers"
                              :total-reviews="represent.data.reviews"
...
import BtnAdd from "../../create-contract/components/BtnAdd.vue";
...
export default {
  ...
  methods: {
    ...
    // Hien thi action them nguoi xem xet, them nguoi ky
    showAddSigner(value) {
      return !value.coordinator;
    },
    // Hien thi action xoa nguoi ky
    showRemoveSigner(value) {
      return !(
        value.data.signers.length < 2 ||
        (value.coordinator &&
          value.coordinator.processStatus ===
            constants.SIGN_PROCCESS_STATUS.PROCESSED)
      );
    },
    // Hien thi action xoa nguoi xem xet
    showRemoveReviewer(value) {
      return !(
        value.data.reviews.length < 2 ||
        (value.coordinator &&
          value.coordinator.processStatus ===
            constants.SIGN_PROCCESS_STATUS.PROCESSED)
      );
    },
    // check disbale title khi nguoi dieu phoi da dieu phoi xong
    checkDisabledCoordinator(value) {
      return (
        value.coordinator &&
        value.coordinator.processStatus ===
          constants.SIGN_PROCCESS_STATUS.PROCESSED
