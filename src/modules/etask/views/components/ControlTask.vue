          ...
          <div class="d-flex">
            ...
            >
              <v-text-field
                ...
                name="txtSearch"
                autofocus
                :placeholder="$t('task-manager.label.name-performer')"
...
import AdvanceSearch from '@/modules/etask/views/components/AdvanceSearch'
...
export default {
  ...
  watch: {
    ...
    /* dateRangeText() {
      ...
      if (this.dateRangeText.length > 0) {
        const newArr = this.dateRangeText.split('~')
        if (newArr.length !== 2) {
          this.errorDateFromTo = 'task-manager.dateInvalid'
          return
        }
        const isValid0 = moment(
          newArr[0].trim(),
          'DD/MM/YYYY',
          true
        ).isValid()
        const isValid1 = moment(
          newArr[1].trim(),
          'DD/MM/YYYY',
          true
        ).isValid()
        ...
        const dateFrom = moment(newArr[0], 'DD/MM/YYYY')
        const dateTo = moment(newArr[1], 'DD/MM/YYYY')
        const diff = dateTo.diff(dateFrom, 'days')
        if (diff < 0) {
          this.errorDateFromTo = 'task-manager.compareDate'
          return
        }
        this.datesPicker = [moment(newArr[0].trim(), 'DD/MM/YYYY').format('YYYY-MM-DD'), moment(newArr[1].trim(), 'DD/MM/YYYY').format('YYYY-MM-DD')]
      }
    }*/
  },
  mounted() {
  ...
  created() {
    this.menu = this.menuTask[(this.$route.params.key === 2 || this.$route.params.key === 8) ? 0 : 2].name
  },
  methods: {
    closeDialog() {
