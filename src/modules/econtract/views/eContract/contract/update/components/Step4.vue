<template>
  <div>
        ...
        <div v-for="(represent, index) in listSigner" :key="index">
          ...
          >
            ...
            <div class="content-panel">
              <!-- Nguoi dieu phoi -->
              <v-expansion-panels
                v-if="isDisplayCoordinator1(represent)"
                v-model="openPanel"
                disabled
      ...
      <v-row justify="space-around">
        ...
        <v-col cols="12" sm="4" md="3">
          <v-row justify="space-between">
            <v-col>
              <v-row justify="space-between">
                <label for="note" class="text--body-1">{{
                  $t("contract.timeExpired")
                }}</label>
                <v-tooltip top content-class="tooltip-top">
                  <template v-slot:activator="{ on, attrs }">
                    <v-icon
      ...
      <v-row class="mt-5">
        <v-col>
          <v-row align="center" class="px-3">
            ...
            <label class="v-label theme--light text--label-1">{{
              $t("contract.day")
            }}</label>
          </v-row>
        </v-col>
    ...
    <div class="d-flex justify-end align-center group-action-step">
      ...
      >
        <span class="text-primary text--button"> {{ $t("cancelLabel") }}</span>
      </v-btn>
      ...
      <v-btn
        ...
        rounded
        color="primary"
        @click="confirmSaveContract(`${docStates.IN_PROGRESS}`)"
      >
        {{ $t("contract.complete") }}
      </v-btn>
      <v-btn
        ...
        rounded
        color="primary"
        @click="confirmCoorditaion(`${docStates.DRAFT}`)"
      >
        {{ $t("sendCoordinatorLabel") }}
      </v-btn>
    </div>
    <dialog-confirm-save
...
import DialogConfirmSave from "../../create-contract/components/DialogConfirmSave.vue";
...
export default {
  ...
  watch: {
    ...
    itemsSign() {
      ...
      var results = _(users).groupBy("email").value();
    ...
    resetcoordination() {
      if (this.resetcoordination) {
        this.setCoordination(false);
  ...
  methods: {
    ...
    ...mapActions("contract", ["setCoordination"]),
    showFileAttach() {
      let check = false;
    ...
    initDataFinalize() {
      ...
      this.documentId = this.$route.params.contractId;
    ...
    // Xac nhan dieu phoi
    confirmCoorditaion(docState) {
      if (!this.validateTimeSignContract()) {
        return;
      }
      this.showConfirmContract = true;
      this.docState = docState;
      this.titleConfirmSave =
        "contract.dialog.titleConfirmCoordinationContract";
    },
    // Confirm save
    confirmSaveContract(docState) {
      if (!this.validateTimeSignContract()) {
        return;
      }
      this.showConfirmContract = true;
      this.docState = docState;
      this.titleConfirmSave =
        docState === this.docStates.DRAFT
          ? "contract.dialog.titleConfirmSaveDraft"
          : "contract.dialog.titleConfirmSaveContract";
    },
    // Luu hop dong
    async saveContract() {
      const formData = {};
      formData.processType = "HDDT";
      formData.documentId = this.documentId;
      var parentDocumentId = null;
      if (this.contractDetail) {
        parentDocumentId =
          this.contractDetail.parentDocumentId ||
          this.contractDetail.documentId;
      }
      formData.parentDocumentId = parentDocumentId;

      formData.documentFormulaId = this.contractType.id;
      formData.title = this.contractName.trim();
      formData.fileMainSign = { ...this.fileContractUploaded };
      delete formData.fileMainSign.url;
      formData.fileAttachments = this.fileAttachsUploaded;
      formData.note = this.note ? this.note.trim() : null;
      formData.signingDeadline = this.timeSignValue ? this.timeSignValue : null;
      formData.expirationDate = this.timeExpiredValue
        ? this.timeExpiredValue
        : null;
      formData.isAgree = this.isAgreeTerm;
      formData.docState = this.docState;
      formData.signReminderDays = this.isRemine ? this.dayRemine : null;
      formData.isSignReminder = this.isRemine != null;
      formData.representGroups = this.buildParamSigner();
      try {
        this.loading = true;
        await ContractService.saveContract(formData);
        var message = "";

        if (this.haveCoordinator) {
          message = "contract.coordinationContractSuccess";
        } else {
          message =
            this.docState === constants.DOC_STATES.DRAFT
              ? "contract.saveDraftSuccess"
              : "contract.saveContractSuccess";
        }
        ...
        // this.$router.push('/app/contracts/created')
      ...
      this.listSigner.map((signator, index) => {
        ...
        if (signator.data.isCoordinator) {
          const { signData } = this.buildDocNotRequest(
            constants.SIGN_USER_TYPE.COORDINATOR,
