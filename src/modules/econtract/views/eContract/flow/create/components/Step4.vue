<template>
  <div>
    <div class="wrap-content-contract">
      ...
      <v-row>
        <v-col>
          <label for="note" class="text--label-1">{{
            $t("contract.note")
          }}</label>
          <v-textarea
            v-model="note"
      ...
      <v-row justify="space-around d-none">
        ...
        <v-col cols="12" sm="4" md="3">
          <v-row justify="space-between">
            <v-col>
              <v-row justify="space-between">
                <label class="text--label-1" for="note">
...
import DialogConfirmSave from "./DialogConfirmSave";
...
export default {
  ...
  methods: {
    ...
    confirmSaveFlow(docState) {
      ...
      this.titleConfirmSave = "flow.dialog.titleConfirmSave";
    },
    // Luu hop dong
    async saveContract() {
      const formData = {};
      ...
      try {
        ...
        var message = "flow.messages.saveSuccess";
