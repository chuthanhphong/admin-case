...
export default {
  ...
  computed: {
    ...
    checkShowInput() {
      return !(this.showInputPhone && !this.signer.phone)
    }
  },
  watch: {
    selectedItem() {
  ...
  methods: {
    ...
    filterUsers() {
      ...
      if (this.signer.userId && this.type && this.listUsers.length > 0) {
        const validSigner = this.listUsers.some(
          user => this.signer.userId === user.userId
        )
        if (!validSigner) {
          this.signer.userId = ''
          this.signer.email = ''
